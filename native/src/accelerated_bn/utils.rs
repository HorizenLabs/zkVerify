...
#[inline(always)]
pub fn decode<T: CanonicalDeserialize>(buf: Vec<u8>) -> Result<T, BnCryptoError> {
    ArkScale::<T>::decode(&mut &buf[..])
        .map_err(|_| BnCryptoError::DecodeError)
        .map(|v| v.0)
}
...
#[inline(always)]
pub fn decode_proj_sw<T: SWCurveConfig>(buf: Vec<u8>) -> Result<SWProjective<T>, BnCryptoError> {
...
#[allow(unused)]
pub fn multi_miller_loop<T: Pairing>(g1: Vec<u8>, g2: Vec<u8>) -> Result<Vec<u8>, BnCryptoError> {
...
#[allow(unused)]
pub fn final_exponentiation<T: Pairing>(target: Vec<u8>) -> Result<Vec<u8>, BnCryptoError> {
    let target = decode::<<T as Pairing>::TargetField>(target)?;
    let res =
        T::final_exponentiation(MillerLoopOutput(target)).ok_or(BnCryptoError::FinalExponError)?;
    Ok(encode(res.0))
}
...
pub fn msm_sw<T: SWCurveConfig>(
    bases: Vec<u8>,
    scalars: Vec<u8>,
) -> Result<Vec<u8>, BnCryptoError> {
    let bases = decode::<Vec<SWAffine<T>>>(bases)?;
    let scalars = decode::<Vec<<T as CurveConfig>::ScalarField>>(scalars)?;
    let res = <SWProjective<T> as VariableBaseMSM>::msm(&bases, &scalars)
        .map_err(|_| BnCryptoError::BaseScalarLengthMismatch)?;
    Ok(encode_proj_sw(&res))
}

#[allow(unused)]
pub fn mul_projective_sw<T: SWCurveConfig>(
    base: Vec<u8>,
    scalar: Vec<u8>,
) -> Result<Vec<u8>, BnCryptoError> {
